    ...
    MimosaAlignAnalysis::MimosaAlignAnalysis(DSession *aSession)
{
  ...
  if (fgInstance)
    Warning("MimosaAlignAnalysis", "object already instantiated");
  else
    fgInstance = this;
...
Bool_t MimosaAlignAnalysis::CheckTrackInGeoLimits(DTrack *aTrack)
{
  ...
  if ((fLimitsX[1] - fLimitsX[0] > 10. && (trackX < fLimitsX[0] || fLimitsX[1] < trackX)) || (fLimitsY[1] - fLimitsY[0] > 10. && (trackY < fLimitsY[0] || fLimitsY[1] < trackY)))
  {
    if (AlignDebug > 1)
      printf("MimosaAlignAnalysis::AlignTracker track at X=%.1f, Y=%.1f, Z=0 rejected because outside limits in X (%.1f-%.1f) or Y (%.1f-%.1f)\n", trackX, trackY, fLimitsX[0], fLimitsX[1], fLimitsY[0], fLimitsY[1]);
    return 0;
  }
                                       ...
                                       bool UseAllHits)
{
  ...
  if (tTracker->GetAlignmentStatus() == 0)
  { // all (prim.+sec.) references to be aligned
  ...
  else if (tTracker->GetAlignmentStatus() == 1)
  { // only secondary references to be aligned
    nSecondaryPlanes = tTracker->GetNVarRef();
    SecondaryPlaneNo = tTracker->GetListVarRef();
  }
  else
  { // only DUTs to be aligned
  ...
  for (Int_t i = 0; i < nSecondaryPlanes; i++)
  {
    ...
    if (AlignDebug)
      printf("MimosaAlignAnalysis::AlignTracker got sec plane %d  pos(%.1f, %.1f) tiltZ %f\n", SecPlane[i]->GetPlaneNumber(), posUold[i], posVold[i], tiltWold[i]);
  ...
  if (AlignDebug > 1)
    printf("MimosaAlignAnalysis::AlignTracker starting the loop over %d events\n", nAlignEvents); // JB 2011/04/18
  fSession->SetEvents(nAlignEvents);
  Bool_t stop = false;
  Int_t counts = 0;
  while (fSession->NextRawEvent() == kTRUE && !stop
         //&& counts<nAlignEvents
  )
  {
    ...
    if (AlignDebug > 1)
      cout << "Reading event " << counts << endl; // JB 2011/04/18
    //===================
    tTracker->Update();
    //===================
    if (AlignDebug > 1)
      printf("MimosaAlignAnalysis::AlignTracker event %d read and %d tracks found\n", fSession->GetCurrentEventNumber(), tTracker->GetTracksN()); // JB 2011/04/18
    ...
    while (iTrack <= tTracker->GetTracksN())
    {                                       // while there is still another track, JB 2009/07/17
      track = tTracker->GetTrack(iTrack++); // tTracker->GetPrincipalTrack();

      // Reject tracks not in the geometrical limits at z=0,
      // JB 2013/06/10
      if (!CheckTrackInGeoLimits(track) || (fChi2Limit > 0.1 && track->GetChiSquare() > fChi2Limit))
      {
        continue;
      }
      ...
      for (Int_t i = 0; i < nSecondaryPlanes; i++)
      { // loop on secondary ref planes
        if (AlignDebug > 2)
          printf("MimosaAlignAnalysis::AlignTracker aligning plane %d with track %d\n", SecPlane[i]->GetPlaneNumber(), iTrack); // JB 2011/04/18
        //====================
        // align in 2D if necessary, but tilt is taken from delta_U vs V only, JB 2007 June
        if (!stopAlign[i])
          fitdone = SecPlane[i]->Align(track, kTRUE, UseAllHits); // stop alignment when it is already good, JB 2009/08/21
                                                                  //        if( !stopAlign[i] ) fitdone = SecPlane[i]->AlignAllHits(track, kTRUE); // stop alignment when it is already good, JB 2009/08/21
        //====================

        if (AlignDebug > 2)
          printf("MimosaAlignAnalysis::AlignTracker status plane %d: stop? %d, mode %d enough? %d\n", SecPlane[i]->GetPlaneNumber(), stopAlign[i], SecPlane[i]->GetAnalysisMode(), SecPlane[i]->GetAlignment()->Enough2D());
        if (!stopAlign[i] && fitdone)
        { // condition changed (the others were obsolete) JB 2011/07/25
          ...
          if (posU[i] - posUold[i] > maxDiffU)
            maxDiffU = posU[i] - posUold[i];
          else if (posU[i] - posUold[i] < minDiffU)
            minDiffU = posU[i] - posUold[i];
          hposu[0]->SetBins(nAlignEvents / 100, 0, nAlignEvents, 600, minDiffU, maxDiffU);
          if (posV[i] - posVold[i] > maxDiffV)
            maxDiffV = posV[i] - posVold[i];
          else if (posV[i] - posVold[i] < minDiffV)
            minDiffV = posV[i] - posVold[i];
          hposv[0]->SetBins(nAlignEvents / 100, 0, nAlignEvents, 600, minDiffV, maxDiffV);
          if (tiltW[i] - tiltWold[i] > maxDiffV)
            maxDiffV = tiltW[i] - tiltWold[i];
          else if (tiltW[i] - tiltWold[i] < minDiffV)
            minDiffV = tiltW[i] - tiltWold[i];
      ...
      if (stop)
        printf("  Alignement can stop now\n");
      if (AlignDebug > 2)
        printf("MAlign: alignement stop=%d\n", stop);
  ...
  while (fSession->NextRawEvent() == kTRUE)
  {
    ...
    for (Int_t it = 1; it <= tTracker->GetTracksN(); it++)
    { // loop on tracks
...
void MimosaAlignAnalysis::StudyDeformation(const Float_t tiniBound, Int_t nEvents, Bool_t fitMode, Float_t minU, Float_t maxU, Float_t minV, Float_t maxV)
{
  ...
  for (Int_t i = 0; i < nSecondaryPlanesDeformation; i++)
  { // loop on secondary ref planes
  ...
  while (fSession->NextRawEvent() == kTRUE)
  {
    ...
    for (Int_t it = 1; it <= tTrackerDeformation->GetTracksN(); it++)
    { // loop on tracks
                                             ...
                                             bool UseAllHits)
{
  ...
  if (modeAuto != 0 && modeAuto != 1)
    modeAuto = 1;

  // Keep the strongest chi2 upper limit
  if (fChi2Limit > 0. && fChi2Limit < chi2Limit)
    chi2Limit = fChi2Limit;
  ...
  if (AlignDebug > 1)
    printf("MimosaAlignAnalysis::AlignTrackerMinuit starting the loop over %d events\n", nAlignEvents);
  fSession->SetEvents(nAlignEvents);
  Int_t counts = 0;
  ...
  while (fSession->NextRawEvent() == kTRUE)
  {
    ...
    if (AlignDebug > 1)
      cout << "Reading event " << counts << endl;
    ...
    if (AlignDebug > 1)
      printf("MimosaAlignAnalysis::AlignTrackerMinuit event %d read and %d tracks found\n", fSession->GetCurrentEventNumber(), tTracker->GetTracksN()); // JB 2011/04/18

    Int_t iTrack = 1;

    while (iTrack <= tTracker->GetTracksN())
    {                                       // while there is still another track
      track = tTracker->GetTrack(iTrack++); // tTracker->GetPrincipalTrack();
      ...
      { // loop on secondary ref planes
        ...
        if (chi2Limit == 0. ||
            ...
            (track->GetChiSquare() >= 0. && track->GetChiSquare() < chi2Limit))
          SecPlane[i]->AlignPrec(track, SecPlane[i]->GetAlignment()->GetBounding(), UseAllHits);

        if (AlignDebug > 2)
          std::cout << "fSession->GetCurrentEventNumber() = " << fSession->GetCurrentEventNumber() << "   SecPlane[i]->GetPrecAlignment()->DataSize() : " << SecPlane[i]->GetPrecAlignment()->DataSize() << "  trackChi2 = " << tTracker->GetTrack(iTrack++)->GetChiSquare() << std::endl;

        if (/*fSession->NextRawEvent() == kFALSE*/ SecPlane[i]->GetPrecAlignment()->DataSize() >= nAlignHitsInPlane && stopAlign[i] == kFALSE)
        { // if enough events for a fit, condition ==nAlignHitsInPlane changed to >=, JB+LC 2013/08/21

          if (AlignDebug)
            std::cout << "DataSize() = " << SecPlane[i]->GetPrecAlignment()->DataSize() << std::endl;

          cout << endl
               << "=============" << endl
               << " Alignment of plane " << SecPlane[i]->GetPlaneNumber() << " with MINUIT and bound=" << SecPlane[i]->GetAlignment()->GetBounding() << endl
               << "=============" << endl
               << endl;
          ...
          if (modeAuto == 0)
          { // if manuall mode, continue, JB 2013/08/29
            Double_t newBound = 0.;
            Char_t answer[100];
            while (0. <= newBound && newBound < 0.1)
            {
              ...
              cout << endl
                   << endl
                   << endl
                   << "Choose a new minimal track-hit distance (um) for next iteration, set a negative number to stop alignment: " << endl
                   << endl;
              cin >> answer;
  ...
  { // loop on secondary ref planes
    ...
    if (i == 0)
    {
      ...
      if (gSystem->mkdir(dirName, 0) == -1)
        cout << "Directory always exist or Cannot create directory " << fSession->GetRunNumber() << std::endl;
      if (gSystem->cd(dirName) == 0)
        cout << "Cannot open repertory " << fSession->GetRunNumber() << std::endl;
    }
    SecPlane[i]->GetAlignment()->Display(fSession->GetRunNumber());
    SecPlane[i]->ShowAlignment();
    SecPlane[i]->GetAlignment()->ShowStoredResolution();

    if (i == nSecondaryPlanes - 1)
      gSystem->cd("..");
  }
...
void MimosaAlignAnalysis::AlignTrackerMinuitLadder(Bool_t modeAuto, const Double_t tiniBounds, Int_t nAlignEvents, Int_t nAlignHitsInPlane, Int_t nAdditionalEvents, Double_t chi2Limit)
{
  ...
  if (numberOfLadders == 0)
    cout << "ERROR !!! No ladder. Must have at least one ladder." << endl;
        ...
        for (Int_t p = startIndex; p < endIndex; ++p)
        {
          ...
          else if (tTracker->GetAlignmentStatus() == 0 && myLadders[k]->GetStatus() > 0)
          {
            ...
            if (myData.size() != 0)
            {
              ...
              if (chi2Limit == 0)
                myLadders[k]->AlignLadder(myData, k, p);
              else if (track->GetChiSquare() <= chi2Limit)
                myLadders[k]->AlignLadder(myData, k, p);
            }
            myData.clear();
          }
          ...
          else if (tTracker->GetAlignmentStatus() >= 2 && myLadders[k]->GetStatus() > 2)
          { // align just the DUT (Status 3)
            std::vector<Double_t> myData;
            myPlane->AlignData(track, tiniBound, myData);
          ...
          if (/*myLadders[k]->GetPlane(startIndex)->GetPrecAlignment()->DataSize()*/ totalHitsInAlignment[k] >= nAlignHitsInPlane && stopAlignLadder[k] == 0)
          {
            ...
            cout << endl
                 << "=============" << endl
                 << " Alignment of Ladder " << k + 1 << " with MINUIT and bound=" << tiniBound << endl
                 << "=============" << endl
                 << endl;
  ...
  for (Int_t l = 0; l < numberOfLadders; ++l)
  { // loop on ladders

    for (Int_t pId = myLadders[l]->GetFirstPlane(); pId < myLadders[l]->GetFirstPlane() + myLadders[l]->GetNumberOfPlanes(); ++pId)
    { // planes in ladder
    ...
    { // while there is still another track
      ...
      for (Int_t k = 0; k < numberOfLadders; ++k)
      {
        ...
        for (Int_t p = startIndex; p < endIndex; ++p)
        {
          ...
          {
            if (chi2Limit == 0)
              myLadders[k]->FillResidus(myData, p);
            else if (track->GetChiSquare() <= chi2Limit)
              myLadders[k]->FillResidus(myData, p);
          }
          myData.clear();
  ...
  for (Int_t k = 0; k < numberOfLadders; ++k)
  {
    ...
    if (myLadders[k]->GetStatus() == 0)
    {
      cout << "Ladder status = 0, writting translations and tilts skipped !" << endl;
      continue;
    }
...
void MimosaAlignAnalysis::AlignLadderMV(Bool_t modeAuto, const Double_t tiniBound, const Double_t boundSlopes, Int_t nAlignEvents, Int_t nGoodTracks, Int_t nAdditionalEvents, Int_t chi2Limit, Int_t mode)
{
  ...
  for (Int_t j = 0; j < numberOfLadders; ++j)
  {
    ...
    if (fLadders[j]->GetMiniVectorsSize() != 0)
      fLadders[j]->ResetMiniVectors(); // Reset Mini-vectors if they already  exist
    // fLadders[j]->UpdateAlignment();
  ...
  while (testNextEvent == 1)
  {
    ...
    if (fLadders[0]->GetMiniVectorsSize() != 0 && fLadders[1]->GetMiniVectorsSize() != 0)
      AssociateMiniVectors(boundDistance, boundSlopes, mode);
    else
    {
    ...
    for (Int_t j = 0; j < numberOfLadders; ++j)
    {
      if (fLadders[j]->IfAssociated() == 0)
        fLadders[j]->ResetMiniVectors(); // Delete all the MiniVector* object and clear the container.
    }
  ...
  if (printResidues == 1)
  {
    ...
    while (fSession->NextRawEvent() == kTRUE)
    {
      ...
      */
      ...
      {
        if (fLadders[j]->IfAssociated() == 0)
          fLadders[j]->ResetMiniVectors(); // Delete all the MiniVector* object and clear the container if MVs not associated.
      }
    ...
    if (nAlignEvents > 0)
    {
      ...
      for (Int_t k = 0; k < numberOfLadders; ++k)
      {
        ...
        for (Int_t i = 1; i <= fLadders[k]->GetNumberOfPlanes(); i++)
        { // loop on secondary ref planes
...
void MimosaAlignAnalysis::AlignTrackerGlobal(const Int_t refPlane, const Int_t alignEvents, const Int_t iterationNumber, const Bool_t alignConstrainsts, const Bool_t trackConstrainsts, const Bool_t multipleScatteringFit)
{
  ...
  for (Int_t i = 0; i < planeNumber; ++i)
  {
    ...
    if (planeReadout > 0)
    {
      ...
      if (mapPlane_OrderZ.count(positionZ_Int) == 0)
        mapPlane_OrderZ[positionZ_Int] = aPlane;
      else
      {
        while (mapPlane_OrderZ.count(positionZ_Int) != 0)
          positionZ_Int += 1;
  ...
  for (; itPlane != mapPlane_OrderZ.end(); ++itPlane)
  {
    ...
    Bool_t isFixed = 0;
    for (int j = 0; j < int(fixedPlanes.size()); ++j)
      if (realPlaneIndex == fixedPlanes[j])
        isFixed = 1;
    ...
    if (isFixed)
      mapFixedPlanesIndex[realPlaneIndex] = index + 1;

    if (!isFixed)
      std::cout << " Pl" << realPlaneIndex << " Mapped to Pl" << index + 1 << std::endl;
    else
      std::cout << " Pl" << realPlaneIndex << " Mapped to Pl" << index + 1 << " Fixed Plane" << std::endl;

    index++;
  }

  if (mapFixedPlanesIndex.count(refPlane) == 0)
    mapFixedPlanesIndex[refPlane] = mapPlanesIndex[refPlane];
  ...
  for (Int_t alignIterations = 0; alignIterations <= iterationNumber; ++alignIterations)
  {
    ...
    if (iterationNumber > 1 && alignIterations == 0)
    {
      ...
      for (Int_t p = 0; p < planeNumber; ++p)
      {
        ...
        for (Int_t i = 0; i < planeDoF; ++i)
        {
          ...
          paramName += "_";
          if (i == 0)
            paramName = "translationX";
          else if (i == 1)
            paramName = "translationY";
          else if (i == 2)
            paramName = "translationZ";
          else if (i == 3)
            paramName = "rotationX";
          else if (i == 4)
            paramName = "rotationY";
    ...
    if (alignIterations != iterationNumber)
      std::cout << "Global Alignment :: Starting Iteration number " << alignIterations << std::endl;
    else
      std::cout << "Re-process tracks for Residuals plots." << std::endl;
    ...
    if (alignIterations != iterationNumber)
      std::cout << "Iteration number " << alignIterations << " --> Initial Alignment Parameters Filled" << std::endl;
    else
      std::cout << "Filling Residuals Plots : --> Initial Alignment Parameters Filled" << std::endl;
    ...
    if (globalAlignment->IfAlignConstrainsts() == true)
      std::cout << "Alignment Constrainsts Enable !" << std::endl;
    else
      std::cout << "Alignment Constrainsts Disable !" << std::endl;

    if (globalAlignment->IfTrackConstrainsts() == true)
      std::cout << "Track Constrainsts Enable !" << std::endl;
    else
      std::cout << "Track Constrainsts Disable !" << std::endl;
    ...
    if (globalAlignment->IfAlignConstrainsts() == true)
    {
      ...
      for (Int_t iterPlane = 1; iterPlane <= planeNumber; ++iterPlane)
        if (iterPlane != refPlaneIndex)
          globalAlignment->FixParameter(iterPlane, 2, ResoAlignParamZ); // To fix all Z positions.
    }
    ...
    */
    ...
    if (alignIterations == 0 || alignIterations == iterationNumber)
      globalAlignment->PrintResiduals();
    ...
    while (testAlignment != 1)
    {
      ...
      if (alignIterations == 0)
        std::cout << "Compute Alignment ? [1/0]" << std::endl;
      else
        std::cout << "Compute New Iteration ? [0/1]" << std::endl;
...
void MimosaAlignAnalysis::fcn(Int_t &n, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
  ...
  while (current)
  {
    ...
    if (AlignDebug >= 3)
    {
      cout << "fcn: got hit position, x=" << CurHit(0) << ", y=" << CurHit(1) << ", z=" << CurHit(2) << endl;
    }
    ...
    if (AlignDebug >= 3)
    {
      cout << "fcn: got track position, x=" << CurPoint(0) << ", y=" << CurPoint(1) << ", z=" << CurPoint(2) << endl;
    }
  ...
  if (iflag == 8)
  {
    if (AlignDebug >= 3)
    {
      fAlignement->PrintRotationMatrix();
    }
...
void MimosaAlignAnalysis::fcnLadder(Int_t &n, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
  ...
  for (Int_t i = startIndex; i < startIndex + numberOfPlanes; ++i)
  {
    ...
    while (current)
    {
      ...
      if (AlignDebug >= 3)
      {
        cout << "fcnLadder: got hit position, x=" << CurHit(0) << ", y=" << CurHit(1) << ", z=" << CurHit(2) << endl;
      }
      ...
      */
      ...
      else if (DPrecAlignMethod == 1)
      {
        ...
        std::cout << "BEWARE DPrecAlignMethod = " << DPrecAlignMethod << "don't exist !" << std::endl;
...
void MimosaAlignAnalysis::fcnLadder2(Int_t &n, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
  ...
  for (Int_t i = startIndex; i < startIndex + numberOfPlanes; ++i)
  {
    ...
    while (current)
    {
      ...
      if (AlignDebug >= 3)
      {
        cout << "fcnLadder2: got hit position, x=" << CurHit(0) << ", y=" << CurHit(1) << ", z=" << CurHit(2) << endl;
      }
      ...
      */
      ...
      for (Int_t j = 0; j < 3; j++)
      {
        ...
        if (j < 2)
          chi2_local_plane = pow((CurPoint(j) - CurHit(j)), 2) / resolutionUV[j]; // Necessary or not ? Can be check :)
...
Double_t MimosaAlignAnalysis::ComputeTotalChiSquareMV(const Double_t *par)
{
  ...
  for (Int_t i = 0; i < int(miniVectorsLadder0.size()); ++i)
  {
    ...
    */
    ...
    if (diffAngleX <= 2.5)
      sigmaV = 3.5;
    else if (diffAngleX > 2.5 && diffAngleX <= 5)
      sigmaV = 3.7;
    else if (diffAngleX > 5 && diffAngleX <= 25)
      sigmaV = 3.9;
    else if (diffAngleX > 25 && diffAngleX <= 30)
      sigmaV = 4.0;
    else if (diffAngleX > 30 && diffAngleX <= 40)
      sigmaV = 4.25;
    else if (diffAngleX > 40 && diffAngleX <= 50)
      sigmaV = 4.4;
    else if (diffAngleX > 50 && diffAngleX <= 60)
      sigmaV = 4.75;
    else if (diffAngleX > 60)
      sigmaV = 5.0;

    if (diffAngleY <= 2.5)
      sigmaU = 3.5;
    else if (diffAngleY > 2.5 && diffAngleY <= 5)
      sigmaU = 3.7;
    else if (diffAngleY > 5 && diffAngleY <= 25)
      sigmaU = 3.9;
    else if (diffAngleY > 25 && diffAngleY <= 30)
      sigmaU = 4.0;
    else if (diffAngleY > 30 && diffAngleY <= 40)
      sigmaU = 4.25;
    else if (diffAngleY > 40 && diffAngleY <= 50)
      sigmaU = 4.4;
    else if (diffAngleY > 50 && diffAngleY <= 60)
      sigmaU = 4.75;
    else if (diffAngleY > 60)
      sigmaU = 5.0;
...
//______________________________________________________________________________
//
DPrecAlign *MimosaAlignAnalysis::AlignMimosa(DPrecAlign *initAlignment, TFile *aCorFile, const std::string MimosaResultDir, Double_t aDistance)
{
  ...
  if (!initAlignment || !(initAlignment->GetDataPoints()->First()))
    Fatal("AlignMimosa", "No initial alignment found or empty!");
  ...
  */
  // cd to result dir
  gSystem->cd(MimosaResultDir.c_str());
  if (AlignDebug)
  ...
  trU = *tra;
  trV = *(tra + 1);
  trW = *(tra + 2); // trW added, JB 2010/09/08
  ...
  while (retour != 11)
  {
    ...
    for (Int_t i = 0; i < 250; i++)
      if (commande[i] == ',')
        commande[i] = ' ';
    if (batch_mode == 0)
      fit->mncomd(commande, retour);
    if (batch_mode == 1)
      fit->mncomd("MINIMIZE", retour);
    arglist[0] = 8;
    fit->mnexcm("cali", arglist, 1, ierflg);
  ...
  */
  ...
  Int_t yyy;
  if (batch_mode == 0)
    cout << " Store the results of alignement? (1/0) " << endl;
  if (batch_mode == 0)
    cin >> yyy;
  if (batch_mode == 1)
    yyy = 1; // force storing in batch, JB 2011/06/17
  Int_t err;
...
DPrecAlign *MimosaAlignAnalysis::AlignPrecMimosaLadder(DLadder *myLadder, Bool_t modeAuto, Double_t aDistance)
{
  ...
  */
  ...
  if (modeAuto == 0 || modeAuto == 1)
    batch_mode = modeAuto; // batch_mode = 1 Minuit auto | batch_mode = 0 Minuit by hand.
  else
    batch_mode = 0;
...
std::string MimosaAlignAnalysis::ShowAndSetParameters()
{
  ...
  while (!(chain.compare("exit") == 0 || chain.compare("mini") == 0))
  {
    ...
    if (chain.compare(0, 4, "exit") == 0 || chain.compare(0, 4, "mini") == 0)
      break;
    ...
    if (input[0].compare("set") == 0)
      validLine = 1;
    else
      validLine = 0;
    if (input[1].compare("par") == 0 && validLine == 1)
      validLine = 1;
    else
      validLine = 0;
...
void MimosaAlignAnalysis::FitInterface(Bool_t modeAuto, const Double_t &aDistance, const Double_t &aSlopeMax)
{
  ...
  if (modeAuto == 0)
  {
    ...
    for (int i = 0; i < 3; ++i)
      par[i] = fAlignement->GetRotations()[i];
    for (int i = 3; i < 6; ++i)
      par[i] = fAlignement->GetTranslation()[i - 3];
...
DPrecAlign *MimosaAlignAnalysis::AlignPrecMimosa(DPrecAlign *initAlignment, Bool_t modeAuto, Double_t aDistance)
{
  ...
  if (!(initAlignment->GetDataPoints()->First()))
    Fatal("AlignMimosa", "No initial alignment found or empty!");
