...
void MimosaAnalysis::InitSession(const Int_t TheRun, const unsigned int ThePlane /*=0*/, Int_t EventBuildingMode, TString ConfigFile /*=""*/, TString ConfigDir /*=""*/, TString DataDir /*=""*/, TString DataFile /*=""*/)
{
  ...
  fWorkingDirectory = fLaunchedDirectory.Data();
  // fWorkingDirectory.c_str() = DTDIR;

  cout << " * TAF is working in: " << fWorkingDirectory.c_str() << endl;
  ...
  // gSystem->ChangeDirectory(fWorkingDirectory.c_str()); // chdir to the correct dir to run
...
void MimosaAnalysis::InitCorPar(Int_t aRun, Int_t aPlane, const char *objType)
{
  ...
  if (strstr(objType, "ladder"))
  { // JB 2014/01/10
    sprintf(CorFileName, "%s/CorPar%d_Lader%d.root", CreateGlobalResultDir().c_str(), aRun, aPlane);
  }
  else
  {
    sprintf(CorFileName, "%s/CorPar%d_%d.root", CreateGlobalResultDir().c_str(), aRun, aPlane); // JB 2011/04/12
  }
  ...
  if (MimoDebug)
    cout << "Testing if Zombie and Opened...";
  ...
  // added JB 2011/04/12
  if (MimoDebug)
    cout << "Testing if some keys are there:" << theCorFile->GetNkeys() << " ...";
  if (theCorFile->GetNkeys() == 0)
  { // if no keys, set as new file
    cout << "There is nothing in the existing CorPar file, treated as new." << endl;
    CorStatus = 2;
  }

  if (MimoDebug)
    cout << "done, CorStatus = " << CorStatus << endl;
...
void MimosaAnalysis::MakeEta(int NEvt)
{
  ...
  for (Int_t iPl = 0; iPl < nTotalPlanes; iPl++)
  {
    ...
    if (MimoDebug)
      printf("MPrep::MakeEta Getting plane %d as %d\n", iPl, planeNo[iPl]);
    RSplane[iPl] = tTracker->GetPlane(planeNo[iPl]);

    if (RSplane[iPl]->GetAnalysisMode() == 1)
    { // STRIPS
  ...
  while (fSession->NextRawEvent() == kTRUE)
  {
    ...
    if (tTracker->GetPlanesStatus() >= 1)
    {
      ...
      { // loop on planes
        ...
        if (nHits > 0 && RSplane[iPl]->GetAnalysisMode() != 3)
        { // ensure there is a hit and no binary output (JB 2013/08/19)
  ...
  for (Int_t iPl = 0; iPl < nTotalPlanes; iPl++)
  {
    ...
    //==============================
    else if (RSplane[iPl]->GetAnalysisMode() != 3)
    { // PIXELS
      EtaFunction->cd(padNo[iPl]);
      gPad->SetFillColor(0);
      hEtaDenU[iPl]->Draw();
...
void MimosaAnalysis::CreateConfig(int RunNumberBigin, int NumberOfFilesPerRun, int RunNumberEnd, int RunNumberStep)
{
  ...
  if (RunNumberBigin == fSession->GetRunNumber())
    Fatal("CreateConfig()", "You can`t load the config you want to modify!");

  if (RunNumberEnd == 0)
    RunNumberEnd = RunNumberBigin;
  if (NumberOfFilesPerRun == 0)
    NumberOfFilesPerRun = (*fSession->GetSetup()).GetRunPar().EndIndex - 1;
  ...
  cout << "Creating new config....based on run " << fSession->GetRunNumber() << " for Run from " << RunNumberBigin << " to " << RunNumberEnd << " with step " << RunNumberStep << " (" << NumberOfFilesPerRun << " files per Run)\n"
       << endl;
  // gSystem->cd(""); //back to fWorkingDirectory.c_str()
...
void MimosaAnalysis::CreateLinks(int RunNumberBigin, int NumberOfFilesPerRun, int RunNumberEnd, int RunNumberStep)
{
  ...
  if (RunNumberEnd == 0)
    RunNumberEnd = RunNumberBigin;
  if (NumberOfFilesPerRun == 0)
    NumberOfFilesPerRun = (*fSession->GetSetup()).GetRunPar().EndIndex;
...
void MimosaAnalysis::PrepareRun(int RunNumberBigin, Option_t *aType, int NumberOfFilesPerRun, int RunNumberEnd, int RunNumberStep)
{
  ...
  if (opt.Contains("all"))
    opt = TString("create,get,link");

  if (opt.Contains("create"))
    CreateConfig(RunNumberBigin, NumberOfFilesPerRun, RunNumberEnd, RunNumberStep);
  ...
  if (opt.Contains("link"))
    CreateLinks(RunNumberBigin, NumberOfFilesPerRun, RunNumberEnd, RunNumberStep);
}
