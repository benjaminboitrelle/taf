...
// Last Modified: VR 2014/06/30 replace DTIR by fWorkingDirectory.c_str() in gSystem->cd() or ChangeDirectory()
                                   ...
                                   Int_t UseHitMap, Int_t WriteMissedHits, Int_t WriteGoodHits)
{
  ...
  if (TString(SaveAlign) == TString("yes"))
    fSession->GetSetup()->GetAnalysisPar().SavePlots = false;

  if (!CheckIfDone("init,clear"))
    return;
  if (!ThePlaneNumber || ThePlaneNumber > fSession->GetTracker()->GetPlanesN())
  {
    Warning("MimosaPro()", "Please set a plane number!");
    return;
  }
  if (!RunNumber)
    Error("MimosaPro", "RunNumber not set! Please run InitSession  first");
  ...
  if (!theCorFile)
    InitCorPar(RunNumber, ThePlaneNumber);
  cout << "InitCorPar OK" << endl;
  gSystem->ChangeDirectory(fWorkingDirectory.c_str()); // VR 2014/06/30 replace DTIR by fWorkingDirectory.c_str()
  ...
  { // Main loop over event

    if (MimoDebug)
      Info("MimosaPro", "Reading event %d", ievt);
    ...
    {
      if (selection->GetMaximum() > 15 * selection->GetBinContent(6))
        MainCanvas->SetLogy(); // JB 2010/04/28
      MainCanvas->Modified();
      MainCanvas->Update();
    ...
    StatusEvent[1]++;
    if (ievt / NeventRangeForEfficiency * NeventRangeForEfficiency == ievt)
    {
      ievt_array++;
    }
    ...
    if (MimoDebug > 1)
      cout << " getting the planes, tracks " << Evt->fT1PlanesN << " and hits" << endl;
    ...
    {
      if (MimoDebug > 0)
        Info("MimosaPro", "Empty event %d: #tracks %d\n", ievt, NbOfTrpl);
      continue;
    }
    ...
    for (Int_t iipl = 0; iipl < TMath::Min(NbOfTrpl, 10); iipl++)
    {
      ...
      if (aTrpl->Tpk != ThePlaneNumber)
      { // if this is not the DUT
        if (MimoDebug > 3)
          Info("MimosaPro", "Ref plane %d, track pos (%.0f, %.0f), dist.2hit (%.1f, %.1f), chi2 %f, slope (%.2f, %.2f)", aTrpl->Tpk, aTrpl->Tu, aTrpl->Tv, aTrpl->Tud, aTrpl->Tvd, aTrpl->Tchi2, aTrpl->TDOX, aTrpl->TDOY);
    ...
    if (((ThePlaneStudied->PFrfr1) < Baselinecut) && (usebaselinecut == kTRUE))
    {
      okT = 0;
      okP = 0;
    }
    ...
    if (okP && okT && NbOfHits < CUT_MaxNbOfHits && NbOfHits >= CUT_MinNbOfHits && NbOfTrpl > 0)
    { // Plane exists with tracks and hits
      ...
      for (Int_t iHit = 0; iHit < NbOfHits; iHit++)
      { // loop on hits

        DAuthenticHit *ahit = (DAuthenticHit *)Hits->At(iHit);
        if (ahit->Hpk != ThePlaneNumber)
          continue; // select only hits in DUT
        StatusEvent[6]++;
        Nhitinevent++;
        NRecHit++;
        ...
        if (MimoDebug > 2)
          Info("MimosaPro", " NofPixelsInCluster is set to %d (in hit %d, max %d)\n", NofPixelsInCluster, ahit->HNNS, MaxNofPixelsInCluster);
        ...
        { // for sparsified readout, JB 2009/05/19
          snSeed = ahit->Hq0;
        }
        if (MimoDebug > 2)
          printf("MimosaPro: snSeed computation with Hsn=%f, Hq0=%f, Hn0=%f, NoiseScope=%f => snSeed=%.1f\n", ahit->Hsn, ahit->Hq0, ahit->Hn0, NoiseScope, snSeed);
        ...
        for (Int_t i = 0; i < NofPixelsInCluster; i++)
        {
          ...
          if ((abs(ahit->HkM[i] - ahit->HkM[0]) == 1 || abs(ahit->HkM[i] - ahit->HkM[0]) == NofPixelInRaw) && ahit->HqM[i] > q2nd)
          {
            q2nd = ahit->HqM[i];
            if (ahit->HnM[i] != 0.0)
            {
        ...
        if (snSeed > sn2nd)
        {
          hS2N2All->Fill(snSeed, sn2nd);
        }
        else
        {
          hS2N2All->Fill(sn2nd, snSeed);
        }
        ...
        else
        {
          ...
          if (ahit->HNNS > 0)
            Warning("MimosaPro", "evt %d, segment WARNING ahit->Hq0 = 0", ievt);
        }
        ...
        if (
            ...
            && (ahit->HsnPulse / ahit->Hsn >= CUT_S2N_seed) && (ahit->HSNneighbour >= CUT_S2N_neighbour) // YV 27/1109 you add these 2 lines if you want S/N cuts
        ...
        { // if hit passes cuts
          ...
          Ngoodhitinevent++;
          if (MimoDebug > 1)
            cout << " found good hit " << Ngoodhitinevent - 1 << " with index " << IndexOfGoodHitsInEvent[Ngoodhitinevent - 1] << " pos " << ahit->Hu << ";" << ahit->Hv << " and dist to track " << ahit->Hu - ahit->Htu << ";" << ahit->Hv - ahit->Htv << endl;
          ...
          if (ahit->Htk >= 0)
          { // if a track was associated in Tree
        ...
        { // if hit does not pass cuts
          if (MimoDebug > 1)
            Info("MimosaPro", "Hit not passing selection cuts, ChargeAroundSeed=%f <> %f, seedCharge=%.1f <> %.0f, hotPixel %d", ChargeAroundSeed, CUT_MaxQ_neighbour, ahit->HqM[0] * calibration, 0., Hotpixel);
        }
      ...
      for (Int_t iHit1 = 0; iHit1 < Ngoodhitinevent; iHit1++)
      { // 1st loop over good hits
        ...
        if (IsAlreadyInDuplicateList)
          continue;

        bool HasDuplicate = false;
        for (Int_t iHit2 = iHit1 + 1; iHit2 < Ngoodhitinevent; iHit2++)
        { // 2nd loop over good hits
      ...
      for (Int_t iHit = 0; iHit < NgoodhitDuplicateCounter; iHit++)
      { // loop over good duplicated hits
      ...
      for (Int_t iHit = 0; iHit < Ngoodhitinevent; iHit++)
        IndexOfGoodHitsInEvent[iHit] = IndexOfGoodHitsInEvent_tmp[iHit];

      hnGOODhit->Fill(Ngoodhitinevent);
      hnahitievt->Fill(ievt, Ngoodhitinevent);

      if (MimoDebug)
        Info("MimosaPro", "End loop on hits, found %d good hits in event %d", Ngoodhitinevent, ievt);
      ...
      for (Int_t iTrk = 0; iTrk < NbOfTrpl; iTrk++)
      { // loop on tracks

        aTrpl = (DTransparentPlane *)Trpl->At(iTrk);
        if (aTrpl->Tpk != ThePlaneNumber)
          continue;                                 // select only tracks in DUT
      ...
      if (MimoDebug > 1)
        Info("MimosaPro", " Number of tracks in EXCLUSION GEOMATRIX %d is %d.", GeoMatrixForTrackCut, Ngoodtrackgeominevent);
      ...
      { // loop on tracks
        ...
        if (MimoDebug > 1)
          Info("MimosaPro", " Track %d has chi2=%f <> %f ?", iTrk, aTrpl->Tchi2, TrackChi2Limit);
        ...
        { // if good track
          ...
          if (MimoDebug)
            Info("MimosaPro", " Is track %d in MIMOSA ? %d", iTrk, TrkCrossMimo);

          //---------------------------------------------------------------
          // if track is in the good geometry range
          if (TrkCrossMimo == kTRUE)
          { // if the track is in MIMOSA
            StatusEvent[5]++;
            ...
            if (Ngoodhitinevent <= 0)
            { // no hit at all, JB 2009/09/08
              nmissh++;
            }
            ...
            { // hit there but too far away, JB 2009/09/08
              nmissthdist++;
            }
    ...
    cout << endl;
            ...
            if (MimoDebug > 1)
              cout << " Nearest hit to track at " << TrackToHitDistance << " <?> " << TrackToHitDistanceLimit << ", hit found = " << aHitIsFound << endl;

            //---------------------------------------------------------------
            // If a good hit near this track was found
            if (aHitIsFound)
            { // if a hit matching the track was found
              StatusEvent[8]++;
              StatusEvent[9]++;
              NofClMatchTrack++;
              ...
              for (Int_t iHit_dup = 0; iHit_dup < NgoodhitDuplicateCounter; iHit_dup++)
              { // loop over good duplicated hits
                if (TheHitIdx == IndexOfDuplicatedHits[iHit_dup][0])
                  IdxDuplicatedMatchedHit = iHit_dup;
              } // end of loop over good duplicated hits
              ...
              // JB 2009/09/01
              if (abs(thehit->Hu - thehit->Hsu) < 5. * PixelSizeU && abs(thehit->Hv - thehit->Hsv) < 5. * PixelSizeV)
              {
              ...
              //-----------------------------
              if (MimoDebug > 1)
                cout << " STARTING user's stuff" << endl;
              ...
              if (MimoDebug)
                Info("MimosaPro", "Done with good matched track-hit study");
            ...
            { // end if no hit matching the track was found
              ...
              if (MimoDebug > 2)
              {
                ...
                { // if there is at least a hit
                  ...
                  if ((thehit->Hsn) != 0.)
                  {
                    cout << thehit->Hq0 / (calibration * thehit->Hsn);
                  }
                  else
                  {
                    cout << 0.;
              ...
              if (TheWriteMissedHits == 1)
              { // if write info for missed hits
                if (MimoDebug)
                  cout << " Writing info for missed event " << ievt << endl;
              ...
              if (Ngoodhitinevent > 0)
              { // if there is at least a hit (thehit is the nearest)
      ...
      if (MimoDebug)
        Info("MimosaPro", "End loop on tracks, found %d good tracks in DUT", NgoodtrackinDUTinevent);

    } // end if Plane exists with tracks and hits
    //---------------------------------------------------------------

    else
    { // if there was no plane or tracks or hits
  ...
  if (MaxNbOfTracksInGeom > -1)
    cout << "-------- Max nb of tracks allowed " << MaxNbOfTracksInGeom << ", in exclusion geomatrix " << GeoMatrixForTrackCut << ": " << exgeomUmin << "<U<" << exgeomUmax << ", " << exgeomVmin << "<V<" << exgeomVmax << endl;
  ...
  if (TString(SaveAlign) == TString("yes"))
    fSession->GetSetup()->GetAnalysisPar().DoTelResolutionMC = false;
  ...
  if (fSession->GetSetup()->GetAnalysisPar().SavePlots)
  {
    ...
    TString EPSName = TString(CreateGlobalResultDir().c_str()) + TString(fOutName) + TString("_tmpFile") + long(NPages) + (".pdf");
  ...
  if (fWriteHistos)
  {
    ...
    AllHist->Close();
    delete AllHist;
    gSystem->cd(fWorkingDirectory.c_str()); // VR 2014/06/30 replace DTIR by fWorkingDirectory.c_str()
  }
...
void MimosaAnalysis::MimosaFakerate(Int_t MaxEvt, Float_t S2N_seed, Float_t S2N_neighbour, Int_t submatrix, Int_t GeoMatrix)
{
  ...
  if (!ThePlaneNumber || ThePlaneNumber > fSession->GetTracker()->GetPlanesN())
  {
    Warning("MimosaFakerate()", "Please set a plane number!");
    return;
  }
  if (!RunNumber)
    Error("MimosaFakerate", "RunNumber not set! Please run InitSession  first");
  ...
  { // Main loop over event
    ...
    if (MimoDebug)
      Info("MimosaFakerate", "Reading event %d", ievt);
    if (ievt / NofCycle * NofCycle == ievt || ievt < 10)
    {
    ...
    { // check to avoid crash due to empty event
      if (MimoDebug)
        Info("MimosaFakerate", "Empty event %d: #hits %d\n", ievt, NbOfHits);
      continue;
    }
    NgoodEvents++;
    ...
    { // loop on hits
      ...
      if (snSeed >= CUT_S2N_seed && ahit->HSNneighbour >= CUT_S2N_neighbour)
      { // if hit passes cuts

        Ngoodhits++;
        Ngoodhitsinevent++;

        if (TrackInMimo(Thegeomatrix, ahit->Hu, ahit->Hv, ThesubmatrixNumber))
        { // if in GEOM area
          NgoodhitsinGEOM++;
      ...
      { // if hit does not pass cuts
        if (MimoDebug > 1)
          Info("MimosaFakerate", "Hit not passing selection cuts, S/N(seed)=%.1f <> %.1f, S/N(neighbours only)=%.1f <> %.1f, in GEOM area ? %d", snSeed, CUT_S2N_seed, ahit->HSNneighbour, CUT_S2N_neighbour, TrackInMimo(Thegeomatrix, ahit->Hu, ahit->Hv, ThesubmatrixNumber));
      }
    ...
    if (MimoDebug)
      Info("MimosaFakerate", "End loop on hits, found %d good hits in event %d", Ngoodhitsinevent, ievt);
  ...
  cout << endl
       << "In area defined by geomatrix " << Thegeomatrix << ", limits in U: " << geomUmin << ", " << geomUmax << ", limits in V: " << geomVmin << ", " << geomVmax << endl;
...
void MimosaAnalysis::MimosaCalibration(Int_t MaxEvt, Float_t S2N_seed, Float_t S2N_neighbour, Float_t ChargeAroundSeedCutMax, Int_t submatrix, Int_t GeoMatrix)
{
  ...
  if (!ThePlaneNumber || ThePlaneNumber > fSession->GetTracker()->GetPlanesN())
  {
    Warning("MimosaCalibration()", "Please set a plane number (1 or 2)");
    return;
  }
  if (!RunNumber)
    Error("MimosaCalibration", "RunNumber not set! Please run InitSession  first");
  ...
  cout << "  min S/N ratio: for seed = " << CUT_S2N_seed << ", for neighbours (without seed) = " << CUT_S2N_neighbour << endl;
  if (fIfCalibration)
    cout << "  max Q for neighbours (without seed) = " << CUT_MaxQ_neighbour << endl;
  ...
  { // Main loop over event
    ...
    if (MimoDebug)
      Info("MimosaCalibration", "Reading event %d", ievt);
    ...
    // Loop over hits BUT keep only the ones in the right plane
    if (MimoDebug > 1)
      cout << " Looping over " << NbOfHits << " hits" << endl;
    for (Int_t iHit = 0; iHit < NbOfHits; iHit++)
    { // loop on hits
      ...
      if (MimoDebug > 1)
        printf("MimosaCalibration: trying hit %d against 1st selection: snSeed=%5.1f >? %5.1f, Qseed=%5.1f >? %5.1f\n", iHit, snSeed, CUT_S2N_seed, ahit->Hq0, CUT_Q_seed);
      //--------------------------------------------------------------
      // If hit passes the first selection cuts
      if (snSeed >= CUT_S2N_seed && (ahit->Hq0 >= CUT_Q_seed)                                                                                                                                // JB 2013/11/08
          && (MinNofPixelsInCluster <= ahit->HNNS && ahit->HNNS <= MaxNofPixelsInCluster)                                                                                                    // JB 2013/09/11
          && TrackInMimo(Thegeomatrix, ahit->Hu, ahit->Hv, ThesubmatrixNumber) && (CUT_MinSeedIndex == CUT_MaxSeedIndex || (CUT_MinSeedIndex <= ahit->Hsk && ahit->Hsk <= CUT_MaxSeedIndex)) // allows to select an index range for the seed pixel (ineffective if minIndex==maxIndex), JB 2013/08/21
          && (CUT_MinSeedCol == CUT_MaxSeedCol ||
              (CUT_MinSeedCol <= (ahit->Hsk % NofPixelInRaw) && (ahit->Hsk % NofPixelInRaw) <= CUT_MaxSeedCol)) // allows to select an col range for the seed pixel (ineffective if minCol==maxCol), JB 2013/08/22
          && (CUT_MinSeedRow == CUT_MaxSeedRow ||
              (CUT_MinSeedRow <= (ahit->Hsk / NofPixelInRaw) && (ahit->Hsk / NofPixelInRaw) <= CUT_MaxSeedRow)) // allows to select an col range for the seed pixel (ineffective if minCol==maxCol), JB 2013/08/22
      ...
      { // if hit passes first cuts
        ...
        if (fIfCalibration)
        { // If goal is calibration, added JB 2014/01/10
          if (MimoDebug > 1)
            printf("MimosaCalibration: trying hit %d against calib peak selection: Qneighbour=%5.1f <? %5.1f\n", iHit, ahit->Hqc - ahit->Hq0, CUT_MaxQ_neighbour);
          //--------------------------------------------------------------
          // If hit in the peak, usefull to determine the calibration constant
          if (ahit->Hqc - ahit->Hq0 < CUT_MaxQ_neighbour)
          { // if hit in the calibration peak

            // Ngoodhits++;
            NgoodhitsinPeak++;
            if (MimoDebug > 1)
              printf("MimosaCalibration: hit %d selected for peak\n", iHit);
        ...
        if (MimoDebug > 1)
          printf("MimosaCalibration: trying hit %d against 2nd selection: snNeighbour=%5.1f >? %5.1f, Qneighbour=%5.1f >? %5.1f\n",
                 iHit, ahit->HSNneighbour, CUT_S2N_neighbour, ahit->Hqc - ahit->Hq0, CUT_MinQ_neighbour);
        //--------------------------------------------------------------
        // If hit passes the second selection cuts
        if (ahit->HSNneighbour >= CUT_S2N_neighbour && (ahit->Hqc - ahit->Hq0 >= CUT_MinQ_neighbour) // JB 2013/11/08
            && (ahit->Hqc >= CUT_Q_cluster)                                                          // JB 2014/01/21
        )
        { // if hit passes full cuts
          ...
          Ngoodhitsinevent++;
          if (MimoDebug > 1)
            printf("MimosaCalibration: hit %d selected as %dth hit of event and %ith hit overall\n", iHit, Ngoodhitsinevent, Ngoodhits);
      ...
      { // if hit does not pass cuts
        if (MimoDebug > 1)
          Info("MimosaCalibration", "Hit not passing selection cuts, S/N(seed)=%.1f <> %.1f, S/N(neighbours only)=%.1f <> %.1f, # pixels %d, in GEOM area ? %d", snSeed, CUT_S2N_seed, ahit->HSNneighbour, CUT_S2N_neighbour, NofPixelsInCluster, TrackInMimo(Thegeomatrix, ahit->Hu, ahit->Hv, ThesubmatrixNumber));
      }
    ...
    if (MimoDebug)
      Info("MimosaCalibration", "End loop on hits, found %d good hits in event %d (total %d good hits)", Ngoodhitsinevent, ievt, Ngoodhits);
...
void MimosaAnalysis::MimosaMiniVectors(Int_t MaxEvt, Int_t TrackHitDist, Short_t plane0, Float_t S2N_seed0, Float_t S2N_neighbour0, Int_t submatrix0, Int_t GeoMatrix0, Short_t plane1, Float_t S2N_seed1, Float_t S2N_neighbour1, Int_t submatrix1, Int_t GeoMatrix1)
{
  ...
  { // loop on planes
    ...
    if (!theCorFiles[iplane])
      InitCorPar(RunNumber, theplanenumber[iplane]);
    theCorFiles[iplane] = theCorFile;

    if (theCorFiles[iplane]->IsOpen() && theCorFiles[iplane]->FindKey("alignement"))
    { // if the CorPar files is already initialized
  ...
  { // Main loop over event
    ...
    if (MimoDebug)
      Info("MimosaMiniVectors", "Reading event %d", ievt);
    ...
    if (MimoDebug > 1)
      cout << " getting the planes, tracks and hits" << endl;
    TClonesArray *Trpl = Evt->GetTransparentPlanes(); // tracks
    TClonesArray *Hits = Evt->GetAuthenticHits();     // hits (all planes)
    TClonesArray *Planes = Evt->GetAuthenticPlanes(); // planes
    Int_t NbOfTrpl = Trpl->GetLast() + 1;             // total # tracks over all planes
    ...
    {
      if (MimoDebug)
        Info("MimosaMiniVectors", "Empty event %d: #tracks %d, #hits %d, #planes %d \n", ievt, NbOfTrpl, NbOfHits, NbOfPlanes);
      continue;
    }
    ...
    for (Int_t iplane = 0; iplane < NbOfPlanes; iplane++)
    {
      ...
      if (aPlane->Ppk == theplanenumber[0])
        ThePlaneStudied[0] = aPlane;
      else if (aPlane->Ppk == theplanenumber[1])
        ThePlaneStudied[1] = aPlane;
    }
    ...
    // and number of hits and tracks are within limits
    if (ThePlaneStudied[0] && ThePlaneStudied[1] && (usebaselinecut == kFALSE || (ThePlaneStudied[0]->PFrfr1 < Baselinecut)) && (usebaselinecut == kFALSE || (ThePlaneStudied[1]->PFrfr1 < Baselinecut)) && NbOfHits < CUT_MaxNbOfHits && NbOfHits >= CUT_MinNbOfHits)
    { // Plane exists with tracks and hits
      StatusEvent[2]++;
      ...
      { // loop on hits
        ...
        { // loop on planes
          ...
          //------------------------------------------
          if (ahit->Hpk == theplanenumber[iplane])
          { // if hit belongs to expected plane
            StatusEvent[6]++;
            Nhitinevent++;
            NRecHit++;
            ...
            if (MimoDebug > 2)
              Info("MimosaMiniVectors", " NofPixelsInCluster is set to %d (in hit %d, max %d)\n", NofPixelsInCluster, ahit->HNNS, maxNofPixelsInCluster[iplane]);
            ...
            //--------------------------------------------------------------
            // If hit passes the selection cut
            if (ChargeAroundSeed >= CUT_MinQ_neighbour && ahit->HqM[0] * calibration > 0.
            ...
            { // if hit passes cuts
              ...
              if (MimoDebug > 2)
                cout << " plane " << theplanenumber[iplane] << " found good hit " << Ngoodhitinevent[iplane] - 1 << " with index " << IndexOfGoodHitsInEvent[iplane][Ngoodhitinevent[iplane] - 1] << " pos " << ahit->Hu << ";" << ahit->Hv << " and dist to track " << ahit->Hu - ahit->Htu << ";" << ahit->Hv - ahit->Htv << endl;
            ...
            { // if hit does not pass cuts
              if (MimoDebug > 2)
                printf(" plane %d, hit %d NOT passing selection cuts, ChargeAroundSeed=%f <> %f, seedCharge=%.1f <> %.0f, NofPixelsInCluster=%d\n", theplanenumber[iplane], iHit, ChargeAroundSeed, CUT_MinQ_neighbour, ahit->HqM[0] * calibration, 0., NofPixelsInCluster);
            }
      ...
      if (MimoDebug)
        Info("MimosaMiniVectors", "End loop on hits, found %d and %d good hits from planes", Ngoodhitinevent[0], Ngoodhitinevent[1]);

      //---------------------------------------------------------------
      // Loop on tracks BUT keep only the ones in the right plane
      if (MimoDebug > 1)
        cout << " looping over " << NbOfTrpl << " tracks" << endl;
      for (Int_t iTrk = 0; iTrk < NbOfTrpl; iTrk++)
      { // loop on tracks

        DTransparentPlane *aTrpl = (DTransparentPlane *)Trpl->At(iTrk);
        if (aTrpl->Tpk != theplanenumber[0])
          continue; // select only tracks in DUT
        ...
        if (MimoDebug > 2)
          cout << " Track " << iTrk << " has chi2=" << aTrpl->Tchi2 << " <?> " << TrackChi2Limit << endl;
        ...
        { // if good track
          ...
          if (MimoDebug > 1)
            cout << " Is track " << iTrk << "in first MIMOSA ? " << TrkCrossMimo << endl;
          ...
          if (MimoDebug > 1)
            cout << " Is track " << iTrk << "in both MIMOSA ? " << TrkCrossMimo << endl;
          ...
          { // if the track is in MIMOSA
            ...
            if (MimoDebug)
              Info("MimosaMiniVectors", " Good (chi2=%f) track %d found in MIMOSA", aTrpl->Tchi2, iTrk);
            ...
            { // loop on planes
              ...
              if (Ngoodhitinevent[iplane] <= 0)
              { // no hit at all, JB 2009/09/08
                nmissh[iplane]++;
              }

              if (MimoDebug > 2)
                cout << "  Trying to associate hits of plane " << theplanenumber[iplane] << ", loop on " << Ngoodhitinevent[iplane] << " candidates." << endl;
              for (Int_t iHit = 0; iHit < Ngoodhitinevent[iplane]; iHit++)
              { // loop over good hits
                ...
                if (MimoDebug > 2)
                  cout << " Good hit " << iHit << " for plane " << theplanenumber[iplane] << " selected at index " << IndexOfGoodHitsInEvent[iplane][iHit] << " with position " << ahit->Hu << ";" << ahit->Hv << " and dist " << adist << " <? " << TrackToHitDistanceLimit << endl;
              ...
              else if (thehit[iplane])
              { // hit there but too far away, JB 2009/09/08
                nmissthdist[iplane]++;
              }

              if (MimoDebug)
                cout << " Plane " << theplanenumber[iplane] << ": Nearest hit to track at " << trackToHitDistance[iplane] << " <?> " << TrackToHitDistanceLimit << ", hit found = " << aHitIsFound[iplane] << endl;
            ...
            if (aHitIsFound[0] && aHitIsFound[1])
            { // if a hit matching the track was found
              StatusEvent[8]++;
              NofClMatchTrack++;
              temp_NofClMatchTrack[ievt_array]++; // for time dependent efficiency
              ...
              if (MimoDebug)
                Info("MimosaMiniVectors", " Two good hits (plane %d, hit %d & plane %d, hit %d) matching the track", theplanenumber[0], thehit[0]->Hhk, theplanenumber[1], thehit[1]->Hhk);
              ...
              if (MimoDebug)
                Info("MimosaMiniVectors", "Done with good matched track-hits study");
            ...
            { // end if no hit matching the track was found
              ...
              if (MimoDebug > 2)
              {
                ...
                { // if there is at least a hit
                  ...
                  if ((thehit[0]->Hsn) != 0.)
                  {
                    cout << thehit[0]->Hq0 / (calibration * thehit[0]->Hsn);
                  }
                  else
                  {
                    cout << 0.;
              ...
              if (Ngoodhitinevent[0] > 0 && thehit[0])
              { // if there is at least a hit (thehit is the nearest)
      ...
      if (MimoDebug)
        Info("MimosaMiniVectors", "End loop on tracks, found %d good tracks in DUT", NgoodtrackinDUTinevent);
    ...
    if (MimoDebug)
      Info("MimosaMiniVectors", "Event %d ended\n", ievt);
...
void MimosaAnalysis::MimosaPro2Planes(Int_t MaxEvt, Int_t TrackHitDist, Short_t plane0, Float_t S2N_seed0, Float_t S2N_neighbour0, Int_t submatrix0, Short_t plane1, Float_t S2N_seed1, Float_t S2N_neighbour1, Int_t submatrix1, Int_t GeoMatrix)
{
  ...
  { // Main loop over event
    ...
    if (MimoDebug)
      Info("MimosaPro2Planes", "Reading event %d", ievt);
    ...
    {
      if (MimoDebug)
        Info("MimosaPro2Planes", "Empty event %d: #tracks %d, #hits %d, #planes %d \n", ievt, NbOfTrpl, NbOfHits, NbOfPlanes);
      continue;
    }
    ...
    { // Plane exists with tracks and hits
      ...
      { // loop on hits
        ...
        { // loop on planes
          ...
          { // if hit belongs to expected plane
            ...
            NofPixelsInCluster = (ahit->HNNS < maxNofPixelsInCluster[iplane]) ? ahit->HNNS : maxNofPixelsInCluster[iplane];
            if (MimoDebug > 2)
              Info("MimosaPro2Planes", " NofPixelsInCluster is set to %d (in hit %d, max %d)\n", NofPixelsInCluster, ahit->HNNS, maxNofPixelsInCluster[iplane]);
            ...
            // If hit passes the selection cut
            if (
                snSeed >= cut_S2N_seed[iplane] && (ahit->HSNneighbour >= cut_S2N_neighbour[iplane]) && (ChargeAroundSeed >= CUT_MinQ_neighbour) && (ahit->HNNS <= maxNofPixelsInCluster[iplane])
      ...
      if (MimoDebug)
        Info("MimosaPro2Planes", "End loop on hits, found %d and %d good hits from planes", Ngoodhitinevent[0], Ngoodhitinevent[1]);
      ...
      { // loop on tracks
        ...
        { // if good track
          ...
          if (MimoDebug > 1)
            cout << " Is track " << iTrk << "in MIMOSA ? " << TrkCrossMimo << endl;
          ...
          { // if the track is in MIMOSA
            ...
            if (MimoDebug)
              Info("MimosaPro2Planes", " Good (chi2=%f) track %d found in MIMOSA", aTrpl->Tchi2, iTrk);
            ...
            { // if a hit matching the track was found
              ...
              if (MimoDebug)
                Info("MimosaPro2Planes", " Good hit found for plane %d, hit %d and for plane %d, hit %d) matching the track", theplanenumber[0], (aHitIsFound[0] ? thehit[0]->Hhk : 0), theplanenumber[1], (aHitIsFound[1] ? thehit[1]->Hhk : 0));
              ...
              for (Short_t iplane = 0; iplane < 2; iplane++)
              { // loop on planes
                if (aHitIsFound[iplane])
                { // if a hit was found for this plane
                  NofPixelsInCluster = (thehit[iplane]->HNNS < maxNofPixelsInCluster[iplane]) ? thehit[iplane]->HNNS : maxNofPixelsInCluster[iplane];
                  MaxNofPixelsInCluster = maxNofPixelsInCluster[iplane];
                  ClusterCharges_compute(thehit[iplane]);
                  ClusterPosition_compute(thehit[iplane], align[iplane]);
              ...
              if (MimoDebug)
                Info("MimosaPro2Planes", "Done with good matched track-hits study");
      ...
      if (MimoDebug)
        Info("MimosaPro2Planes", "End loop on tracks, found %d good tracks in DUT", NgoodtrackinDUTinevent);
    ...
    if (MimoDebug)
      Info("MimosaPro2Planes", "Event %d ended\n", ievt);
...
void MimosaAnalysis::MimosaVertex(Int_t MaxEvt, Short_t plane1, Short_t plane2)
{
  ...
  if (!CheckIfDone("init,clear"))
    return;
  if (!RunNumber)
    Error("MimosaVertex", "RunNumber not set! Please run InitSession  first");
  ...
  { // Main loop over event
    ...
    if (MimoDebug)
      Info("MimosaVertex", "Reading event %d", ievt);
    ...
    {
      if (MimoDebug)
        Info("MimosaVertex", "Empty event %d: #tracks %d\n", ievt, NbOfTrpl);
      continue;
    }
    ...
    // if Both TransparentPlane and AuthenticPlane exist : (JB, Sept 2008)
    // and number of hits and tracks are within limits
    if (Nhitinevent < CUT_MaxNbOfHits && Nhitinevent >= CUT_MinNbOfHits && NbOfTrpl > 0)
    { // Plane exists with tracks and hits
      StatusEvent[2]++;
      ...
      if (MimoDebug)
        Info("MimosaPro", "End loop on tracks, found %d good tracks", Ngoodtrackinevent);
...
void MimosaAnalysis::MimosaVertexFinder(Int_t MaxEvt, Int_t submatrix, Int_t GeoMatrix, Float_t chi2Cut)
{
  ...
  if (!ThePlaneNumber || ThePlaneNumber > fSession->GetTracker()->GetPlanesN())
  {
    Warning("MimosaVertexFinder()", "Please set a plane number!");
    return;
  }
  if (!RunNumber)
    Error("MimosaVertexFinder", "RunNumber not set! Please run InitSession  first");
  ...
  { // Main loop over event
    ...
    if (MimoDebug)
      Info("MimosaVertexFinder", "Reading event %d", ievt);
    if (ievt / NofCycle * NofCycle == ievt || ievt < 10)
    {
...
void MimosaAnalysis::MimosaProLadder(Int_t MaxEvt, Int_t TrackHitDist, Float_t S2N_seed, Float_t S2N_neighbour, Int_t submatrix, Int_t GeoMatrix, Option_t *SaveAlign, Int_t UseHitMap, Int_t WriteMissedHits, Int_t WriteGoodHits)
{
  ...
  if (!CheckIfDone("init,clear"))
    return;
  if (!TheLadderNumber || TheLadderNumber > fSession->GetTracker()->GetNumberOfLadders())
  {
    Warning("MimosaPro()", "Please set a ladder number!");
    return;
  }
  if (!RunNumber)
    Error("MimosaPro", "RunNumber not set! Please run InitSession  first");
  ...
  if (!theCorFile)
    InitCorPar(RunNumber, TheLadderNumber, "ladder");
  cout << "InitCorPar OK" << endl;
  gSystem->ChangeDirectory(fWorkingDirectory.c_str()); // VR 2014/06/30 replace DTIR by fWorkingDirectory.c_str()
  ...
  { // Main loop over event
    ...
    { // Plane exists with tracks and hits
      ...
      { // loop on hits
        ...
        DAuthenticHit *ahit = (DAuthenticHit *)Hits->At(iHit);
        if (IsPlaneInLadder(ahit->Hpk, TheLadderNumber))
          continue; // select only hits in DUT
        StatusEvent[6]++;
        Nhitinevent++;
        NRecHit++;
      ...
      { // loop on tracks
        ...
        aTrpl = (DTransparentPlane *)Trpl->At(iTrk);
        if (IsPlaneInLadder(aTrpl->Tpk, TheLadderNumber))
          continue;                                 // select only tracks in DUT
        TrackParameters_compute(aTrpl, alignement); // compute tu,tv
        ...
        { // if good track
          ...
          { // if the track is in MIMOSA
            ...
            { // if a hit matching the track was found
              ...
              ClusterPosition_compute(thehit, alignement); // JB 2010/06/03
              if (MimoDebug > 1)
                cout << " hit   in plane (Plane frame) " << hU << ", " << hV << ", " << hW << "." << endl;
...
void MimosaAnalysis::MimosaImaging(Int_t MaxEvt, Int_t submatrix, Int_t GeoMatrix, Double_t theta_init, Int_t Npeak, Double_t S, Double_t W, Bool_t FirstLoop, Bool_t chooseFit)
{
  ...
  if (!ThePlaneNumber || ThePlaneNumber > fSession->GetTracker()->GetPlanesN())
  {
    Warning("MimosaImaging()", "Please set a plane number (1 or 2)");
    return;
  }
  if (!RunNumber)
    Error("MimosaImaging", "RunNumber not set! Please run InitSession  first");
  ...
  { // if all the code is performed
    ...
    for (Int_t ievt = MinEvent; ievt <= MaxEvent; ievt++)
    { // First main loop over events

      if (MimoDebug)
        Info("MimosaImaging", "Reading event %d", ievt);
      ...
      { // loop on hits
        ...
        if (fIfImaging)
        {
          ...
          { // loop over thetaCut
            ...
            if ((ImHu >= geomUmin) && (ImHu <= geomUmax) && (ImHv >= geomVmin) && (ImHv <= geomVmax))
            { // cut over theta_init
      ...
      if (MimoDebug)
        Info("MimosaImaging", "End loop on hits, found %d good hits in event %d (total %d good hits)", Ngoodhitsinevent, ievt, Ngoodhits);

      if (ievt / 10000.0 == int(ievt / 10000.0) && ievt != 0)
        cout << "Number of studied events: " << ievt << endl;
  ...
  {
    if (divisionU >= -1 && divisionU <= 0)
      geomUmax = 0.0 * pitchImHu;
    else if (divisionU > 0)
      geomUmax = (int(geomUmax / pitchImHu) + 1) * pitchImHu;
    else if (divisionU < -1)
      geomUmax = (int(geomUmax / pitchImHu) + 0) * pitchImHu;
  }
  ...
  {
    if (divisionU >= -1 && divisionU <= 0)
      geomUmin = -1.0 * pitchImHu;
    else if (divisionU > 0)
      geomUmin = (int(geomUmin / pitchImHu) + 0) * pitchImHu;
    else if (divisionU < -1)
      geomUmin = (int(geomUmin / pitchImHu) - 1) * pitchImHu;
  }
  ...
  {
    if (divisionV >= -1 && divisionV <= 0)
      geomVmax = 0.0 * pitchImHv;
    else if (divisionV > 0)
      geomVmax = (int(geomVmax / pitchImHv) + 1) * pitchImHv;
    else if (divisionV < -1)
      geomVmax = (int(geomVmax / pitchImHv) + 0) * pitchImHv;
  }
  ...
  {
    if (divisionV >= -1 && divisionV <= 0)
      geomVmin = -1.0 * pitchImHv;
    else if (divisionV > 0)
      geomVmin = (int(geomVmin / pitchImHv) + 0) * pitchImHv;
    else if (divisionV < -1)
      geomVmin = (int(geomVmin / pitchImHv) - 1) * pitchImHv;
  }
  ...
  {
    if (((geomUmax - geomUmin) / NofEffectivePitchU) - pitchImHu > 0)
      NofEffectivePitchU += 1;
    else if (((geomUmax - geomUmin) / NofEffectivePitchU) - pitchImHu < 0)
      NofEffectivePitchU -= 1;
  }
  ...
  {
    if (((geomVmax - geomVmin) / NofEffectivePitchV) - pitchImHv > 0)
      NofEffectivePitchV += 1;
    else if (((geomVmax - geomVmin) / NofEffectivePitchV) - pitchImHv < 0)
      NofEffectivePitchV -= 1;
  }
  ...
  TString TheOutFileName = TString(fWorkingDirectory.c_str()) + TString("file_out_91893.out");
  ...
  { // Main loop over event
    ...
    { // loop on hits
      ...
      if (fIfImaging)
      {
        ...
        if ((ImHu >= geomUmin) && (ImHu <= geomUmax) && (ImHv >= geomVmin) && (ImHv <= geomVmax))
        { // cut over the region of interest
          ...
          if ((ahit->Hq0 >= CUT_Q_seed)                                                       // JB 2013/11/08
              ...
              && (CUT_MinSeedRow == CUT_MaxSeedRow ||
                  (CUT_MinSeedRow <= (ahit->Hsk / NofPixelInRaw) && (ahit->Hsk / NofPixelInRaw) <= CUT_MaxSeedRow)) // allows to select an col range for the seed pixel (ineffective if minCol==maxCol), JB 2013/08/22
              && (CUT_MinNbOfHits <= NbOfHits && NbOfHits <= CUT_MaxNbOfHits) && (ahit->Hqc >= CUT_Q_cluster)       // JB 2014/01/21
          )
          {
